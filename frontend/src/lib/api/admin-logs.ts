...
export const adminLogsApi = {
  // 获取操作日志列表
  getOperationLogs: async (params?: AdminOperationLogListParams): Promise<AdminOperationLogListResponse> => {
  ...
  // 获取单个操作日志详情
  getOperationLog: async (id: number): Promise<AdminOperationLogDetailResponse> => {
    return request<AdminOperationLogDetailResponse>(`/admin/operation-logs/${id}`);
  },

  // 获取操作统计
  getOperationStats: async (params?: AdminOperationStatsParams): Promise<AdminOperationStatsResponse> => {
    const searchParams = new URLSearchParams();
  ...
  // 获取登录日志列表
  getLoginLogs: async (params?: LoginLogListParams): Promise<LoginLogListResponse> => {
    const searchParams = new URLSearchParams();
