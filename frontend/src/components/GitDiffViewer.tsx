...
export const GitDiffViewer: React.FC<GitDiffViewerProps> = ({
  ...
  const loadDiffSummary = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiService.tasks.getTaskGitDiff(taskId, { include_content: false });
      // 确保数据完整性
      ...
      setError(error instanceof Error ? error.message : "Failed to load git diff");
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadFileContent = async (filePath: string) => {
    ...
    try {
      ...
      const response = await apiService.tasks.getTaskGitDiffFile(taskId, { file_path: filePath });
      setFileContents(prev => new Map(prev).set(filePath, response.data.diff_content));
    } catch (error) {
      ...
      <CardHeader>
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-1">
            ...
            <span>{startBranch} → {workBranch}</span>
          </div>
          <div className="flex gap-4">
            <span className="text-green-600">+{diffSummary.total_additions || 0}</span>
            <span className="text-red-600">-{diffSummary.total_deletions || 0}</span>
            <span>{diffSummary.total_files || 0} {t("gitDiff.filesChanged")}</span>
          </div>
        </div>
      </CardHeader>
        ...
        <Tabs defaultValue="summary" className="w-full">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-blue-600">{diffSummary.total_files || 0}</div>
                  <div className="text-sm text-muted-foreground">{t("gitDiff.filesChanged")}</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-green-600">+{diffSummary.total_additions || 0}</div>
                  <div className="text-sm text-muted-foreground">{t("gitDiff.additions")}</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-red-600">-{diffSummary.total_deletions || 0}</div>
                  <div className="text-sm text-muted-foreground">{t("gitDiff.deletions")}</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-gray-600">{diffSummary.commits_ahead || 0}</div>
                  <div className="text-sm text-muted-foreground">{t("gitDiff.commitsAhead")}</div>
                </CardContent>
              </Card>
            </div>
          </TabsContent>
          ...
          <TabsContent value="files" className="space-y-4">
            <div className="space-y-2">
              {safeFiles.length > 0 ? safeFiles.map((file) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className={`text-xs ${getStatusColor(file.status)}`}>
                          ...
                          <span className="ml-1">{t(`gitDiff.status.${file.status}`)}</span>
                        </Badge>
                        {file.is_binary && (
