...
export function TaskExecutionLog({
  ...
  const [executionLog, setExecutionLog] = useState<TaskExecutionLog | null>(null);
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Badge 
              variant="outline" 
              className={`text-xs ${getStatusColor(executionLog.status)}`}
            >
              {getStatusIcon(executionLog.status)}
                             <span className="ml-1">
                 {t(`taskConversation.execution.status.${executionLog.status}`)}
               </span>
            </Badge>
      ...
      <CardContent className="space-y-4">
        {/* 执行信息 */}
           ...
           <div>
             <span className="text-gray-500">{t('taskConversation.execution.info.started')}:</span>
             <span className="ml-2">{formatTime(executionLog.started_at)}</span>
           </div>
           <div>
             <span className="text-gray-500">{t('taskConversation.execution.info.completed')}:</span>
             <span className="ml-2">{formatTime(executionLog.completed_at)}</span>
           </div>
           <div>
             <span className="text-gray-500">{t('taskConversation.execution.info.duration')}:</span>
             <span className="ml-2">
               {formatDuration(executionLog.started_at, executionLog.completed_at)}
             </span>
           </div>
           <div>
             <span className="text-gray-500">{t('taskConversation.execution.info.workspace')}:</span>
        ...
        {executionLog.error_message && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
               ...
               <span className="font-medium text-red-700">{t('taskConversation.execution.info.errorMessage')}</span>
             </div>
        ...
        {executionLog.docker_command && (
          <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
               ...
               <span className="font-medium text-gray-700">{t('taskConversation.execution.info.dockerCommand')}</span>
             </div>
        ...
        {executionLog.commit_hash && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
               ...
               <span className="font-medium text-green-700">{t('taskConversation.execution.info.commitHash')}</span>
             </div>
        ...
        {executionLog.execution_logs && (
          <div>
               ...
               <span className="font-medium text-gray-700">{t('taskConversation.execution.info.executionLogs')}</span>
               <Button
                 variant="ghost"
                 size="sm"
