...
interface RepomixFormSheetProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function RepomixFormSheet({
  isOpen,
  onClose,
  onSuccess,
}: RepomixFormSheetProps) {
  const { t } = useTranslation();

  const [formData, setFormData] = useState<RepomixConfig & { name: string }>({
    name: "repomix",
  });
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = t("mcp.templates.repomix.form.fields.name.required");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const config = generateRepomixConfig();
      ...
      logError(error, "Failed to create Repomix MCP configuration");
      ...
      <FormSheetContent className="w-[600px] sm:max-w-[600px]">
        <FormSheetHeader>
          <FormSheetTitle className="flex items-center gap-2">
            ...
            {t("mcp.templates.repomix.form.title")}
          </FormSheetTitle>
          <FormSheetDescription>
            {t("mcp.templates.repomix.form.description")}
          </FormSheetDescription>
        </FormSheetHeader>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="name">
                        {t("mcp.templates.repomix.form.fields.name.label")}{" "}
                      ...
                      <Input
                        ...
                        type="text"
                        placeholder={t(
                          "mcp.templates.repomix.form.fields.name.placeholder"
                        )}
                        value={formData.name}
                        onChange={(e) =>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t("mcp.templates.repomix.form.fields.name.help")}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <FormSheetFooter>
          <Button type="submit" disabled={loading} onClick={handleSubmit}>
            {loading
              ...
              : t("mcp.templates.repomix.form.submit")}
          </Button>
        </FormSheetFooter>
      </FormSheetContent>
    </FormSheet>
