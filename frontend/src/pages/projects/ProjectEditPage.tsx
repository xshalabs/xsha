...
const ProjectEditPage: React.FC = () => {
  ...
  usePageTitle(project ? `${t('projects.edit')} - ${project.name}` : t('projects.edit'));
  ...
  useEffect(() => {
    const loadProject = async () => {
      ...
      try {
        ...
        alert(error instanceof Error ? error.message : t('projects.messages.loadFailed'));
        navigate('/projects');
      } finally {
        setLoading(false);
      }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-2">
            {t('projects.edit_description')} - {project.name}
          </p>
        </div>
